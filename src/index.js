...
const KENDRA_VOICE_PROFILE = [
  "Write in Kendra's first person voice.",
  "Her voice is warm, feminine, confident, playful, sensual, conversational, polished, and personal.",
  "Use everyday language, contractions, short natural sentences, and a human rhythm.",
  "She can be flirty, teasing, affectionate, soft, direct, or lightly cheeky when appropriate, but never sounds scripted.",
  "Avoid poetry, romance novel language, luxury ad copy, corporate language, therapy language, and generic AI phrasing.",
  "Avoid flowery metaphors, dramatic imagery, and overexplaining.",
  "Never refer to Kendra as she, her, Kendra, or Kendra Bexly inside user facing copy unless a signature or subject line specifically calls for her name.",
  "Keep attraction, chemistry, generosity, care, discretion, anticipation, and standards natural and non transactional.",
  "Never invent memories, personal facts, client details, promises, prices, offers, events, or experiences.",
  "Kendra does not drink coffee. Never say or imply that I drink, want, order, make, need, crave, or have coffee. Do not use coffee as generic morning, lifestyle, date, or routine filler.",
  "Do not use hyphens, em dashes, or en dashes in generated prose. Use commas, periods, colons, or natural sentence breaks instead."
].join(" ");
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/admin/x/reply/generate" && request.method === "POST") {
      ...
      const ai = await env.AI.run("@cf/meta/llama-3.1-8b-instruct-fp8", { messages:[
        { role:"system", content:KENDRA_VOICE_PROFILE+" Draft one natural reply to an X user who responded to my post. Keep the conversation flowing. No labels or quotation marks. Return only the reply." },
    ...
    if (url.pathname === "/api/admin/x/generate" && request.method === "POST") {
      ...
      const format = String(data.format || "").trim();
      const sexualTone = String(data.sexual_tone || "suggestive");
      const contentStyle = String(data.content_style || "").trim();
      const selectedStyles = style.split(",").map(x=>x.trim()).filter(Boolean);
      const contentStyleGuide = ({
        "Personal / Lifestyle":"Make the post clearly personal and lifestyle led, centered on my taste, routine, preference, mood, or point of view.",
        "Conversational Question":"Build the post around one natural, easy to answer question that invites conversation without sounding like engagement bait.",
        "Helpful Insight":"Share one useful observation, tip, standard, or takeaway in my voice. It should feel practical, not instructional or corporate.",
        "Behind the Scenes":"Give a believable behind the scenes glimpse into preparation, mindset, routine, planning, or how I approach my work, without inventing facts.",
        "Website / Service Mention":"Naturally mention my website, availability, booking experience, or companionship offering without sounding like an ad or hard sell.",
        "Engaging Observation":"Lead with a relatable observation about attraction, dating, chemistry, manners, confidence, generosity, or everyday behavior.",
        "Light Personality":"Let my personality carry the post. Make it playful, casual, opinionated, cheeky, or charming rather than promotional.",
        "Story / Experience":"Use a tiny first person story shaped like setup, moment, takeaway. Do not invent a specific client, event, or memory that was not provided.",
        "Community Conversation":"Frame the post as a genuine conversation with my audience, inviting opinions or shared perspectives in a natural way.",
        "Soft Call to Action":"End with a subtle next step such as visiting my site, starting a conversation, or making plans, without pressure or sales language."
      })[contentStyle] || "";
    ...
    if (url.pathname === "/api/admin/ad-copy/generate" && request.method === "POST") {
      try {
        ...
        const length = String(data.length || "Medium").trim().slice(0, 40);
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        const draft = await env.DB.prepare(`
          SELECT ed.id, ed.subject, ed.body, ed.email_type, ed.status, ed.client_id, ed.date_request_id, c.email, c.phone, c.first_name
          FROM email_drafts ed
...
I just wanted to say I really enjoyed our time together. Thank you for making it such an easy, enjoyable date. I hope you made it home safely. 💋`;
        if(env.AI){
          try{
            ...
            const ai=await env.AI.run("@cf/meta/llama-3.1-8b-instruct-fp8",{messages:[
              {role:"system",content:KENDRA_VOICE_PROFILE+" Write a short private follow up immediately after a successfully completed date. Sound appreciative, personal, and lightly flirty. Do not sound like customer service. Do not pressure him to book again. Never invent memories or details."},
    ...
    if (url.pathname === "/api/admin/retention/follow-up-draft" && request.method === "POST") {
      try {
        ...
        if (env.AI) {
          try {
            const ai = await env.AI.run("@cf/meta/llama-3.1-8b-instruct-fp8",{messages:[
              {role:"system",content:KENDRA_VOICE_PROFILE+" Write a short private retention follow-up email for a client who has already completed a date with me. Sound personal, lightly flirty, and never like mass marketing. Do not invent memories or details. Do not pressure the client to book. Protect my pricing: do not invent or reduce rates and do not promise a discount. If an incentive is appropriate, prefer mentioning that I may have something special available rather than naming an unverified offer. No poetic language. No hyphens, em dashes, or en dashes. Return only the email body without a signature."},
