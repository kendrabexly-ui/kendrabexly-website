...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/admin/x/library") {
      ...
      if(request.method==="GET"){try{await env.DB.prepare("ALTER TABLE x_content_library ADD COLUMN last_used_at TEXT").run();}catch(e){}const q=await env.DB.prepare("SELECT id,title,content,content_style,CASE WHEN image_base64 IS NULL THEN 0 ELSE 1 END AS has_image,mime_type,file_name,created_at,updated_at,last_used_at FROM x_content_library ORDER BY id DESC LIMIT 100").all();return Response.json({ok:true,items:q.results||[]});}
